...
Confirm the time zone, logon type, and target host.
Review EDR and identity-provider telemetry for the same time window.
Check the IP address using approved reputation sources.
Review MFA results and impossible-travel alerts.
Search for password resets, privilege use, and lateral movement.
Escalate according to the organization's incident-response process if necessary.
